...
openApiGenerate {
    val openapiGroup = "${rootProject.group}.orders.api.v1"
    generatorName.set("kotlin")
    packageName.set(openapiGroup)
    apiPackage.set("$openapiGroup.api")
    modelPackage.set("$openapiGroup")
    invokerPackage.set("$openapiGroup.invoker")
    inputSpec.set("$rootDir/spec/api-spec-orders.yaml")
...
docker {
    springBootApplication {
        ...
        images.set(listOf("ilyabikmetov/microservice-order:$version"))
...
tasks.jar {
    manifest {
        attributes["Main-Class"] = "ru.ibikmetov.microservices.OrdersApplication"
    }
}
