...
openApiGenerate {
    val openapiGroup = "${rootProject.group}.places.api.v1"
    generatorName.set("kotlin")
    packageName.set(openapiGroup)
    apiPackage.set("$openapiGroup.api")
    modelPackage.set("$openapiGroup")
    invokerPackage.set("$openapiGroup.invoker")
    inputSpec.set("$rootDir/spec/api-spec-places.yaml")
...
docker {
    springBootApplication {
        ...
        images.set(listOf("ilyabikmetov/microservice-places:$version"))
...
tasks.jar {
    manifest {
        attributes["Main-Class"] = "ru.ibikmetov.microservices.PlacesApplication"
    }
}
