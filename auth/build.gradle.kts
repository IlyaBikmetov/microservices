...
openApiGenerate {
    val openapiGroup = "${rootProject.group}.auth.api.v1"
    generatorName.set("kotlin")
    packageName.set(openapiGroup)
    apiPackage.set("$openapiGroup.api")
    modelPackage.set("$openapiGroup")
    invokerPackage.set("$openapiGroup.invoker")
    inputSpec.set("$rootDir/spec/api-spec-auth.yaml")
...
docker {
    springBootApplication {
        ...
        images.set(listOf("ilyabikmetov/auth-service:$version"))
...
tasks.jar {
    manifest {
        attributes["Main-Class"] = "ru.ibikmetov.auth-service.MicroservicesApplication"
    }
}
